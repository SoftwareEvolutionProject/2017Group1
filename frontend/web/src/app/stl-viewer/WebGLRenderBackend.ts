...
export class WebGLRenderBackend {
    ...
    screen_vs;
    screen_fs;
    gradient_background_vs;
    gradient_background_fs;
    frame_vs;
    frame_fs;
    solid_vs;
    solid_fs;
        ...
        function sortRenderList(rlist) {
            ...
            for (const mesh of rlist) {
                ...
                if ((mesh.material || defaultMaterial).transparency > 0 || mesh.hasTexture() && mesh.texture.hasTransparency) {
                    ...
                    new Math3D().transformVectors(transformMatrix, mesh.c, mesh.c);
     ...
     * @private
     */
    renderColorPass(renderList, transformMat4, normalMat3, renderMode, defaultMaterial, sphereMap, isCullingDisabled) {
        ...
        for (const mesh of renderList) {
            ...
            if (texture && !texture.compiled) {
                this.compileTexture(texture, null);
            }
     ...
     * @private
     */
    compileTexture(texture, genMipmap) {
        if (!texture.hasData()) {
            return false;
        }
